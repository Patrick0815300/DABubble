...
export class FileUploadThreadService {
  ...
  uploadFileThread(file: File, userId: string, onProgress: (progress: number) => void): Promise<{ url: string, fileName: string }> {
    return new Promise((resolve, reject) => {
      const storage = getStorage();
      const storageRef = ref(storage, `thread/${userId}/${file.name}`);
